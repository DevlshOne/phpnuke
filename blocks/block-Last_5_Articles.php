...
$querylang = "WHERE status = 'publish'";
...
$result = $db->query("SELECT sid, title, comments, counter , article_url, time, cat_link FROM ".ARTICLES_TABLE." $querylang ORDER BY time DESC LIMIT ?,?", $params);
...
if($db->count())
{
	foreach($result as $row)
	{
		...
		$time = $row['time'];
		$llink = LinkToGT(articleslink($sid, $title, $article_url, $time, $cat_link));

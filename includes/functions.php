			...
			foreach($cache_systems as $cache_system_name => $cache_system)
			{
				if($cache_system_name != "" && $cache_system['main_id'] != "" && $cache_system['table'] != "")
				{
					if((!$cache->isCached($cache_system_name) OR in_array($mode, array("$cache_system_name",'all'))) && !in_array("$cache_system_name", $exept_caches))
					{
						...
						$results = $db->query("SELECT * FROM ".$cache_system['table']." WHERE ".$cache_system['where']." ORDER BY ".$cache_system['main_id']." ".$cache_system['order']."");
...
function makePass($cid, $custum_options = array(), $google_recaptcha = false)
{
	...
	if(($nuke_configs['seccode_type'] == 2 || $google_recaptcha) && $nuke_configs['google_recaptcha_sitekey'] != '')
	{
		...
		<script src=\"https://www.google.com/recaptcha/api.js?hl="._GOOGLE_RECAPTCHA_LANG."\"></script>
		<div id=\"security_code_$cid\" class=\"security_code_container\"><div class=\"g-recaptcha\" data-sitekey=\"".$nuke_configs['google_recaptcha_sitekey']."\"></div></div>";
	}
...
function is_user($username='')
{
	...
	if($username != '')
	{
		$result = $db->query("SELECT ".$users_system->user_fields['user_id']." FROM ".$users_system->users_table." WHERE ".$users_system->user_fields['username']." = '$username' ".(($users_system->user_fields['common_where'] != '') ? " AND ".$users_system->user_fields['common_where']:"")."");
...
function show_modules_boxes($module_name, $active_boxes=array(), $html_output="prev", $special_bids=array(), $except_bids=array())
{
	global $nuke_modules_cacheData, $file, $theme_setup;
	...
	$all_module_boxes = (isset($all_module_boxes[$file])) ? explode("|", $all_module_boxes[$file]):array();
		...
		switch($config)
		{
			...
			case"PHPNukeAdmin":
				$toolbar			= "[
					...
					{ name: 'links', items: [ 'Link', 'Unlink', 'Anchor' ] },
					'/',
					{ name: 'insert', items: [ 'CodeSnippet', 'Image', 'Flash', 'Html5audio', 'Video', 'Table', 'HorizontalRule', 'Smiley', 'SpecialChar', 'PageBreak', 'Iframe' ] },
					{ name: 'styles', items: [ 'Styles', 'Format', 'Font', 'FontSize' ] },
					{ name: 'colors', items: [ 'TextColor', 'BGColor' ] },
					{ name: 'tools', items: [ 'Maximize', 'ShowBlocks' ] },
					{ name: 'others', items: [ '-' ] },
...
function get_theme()
{
    ...
    if (is_user() && ($modname != "Your_Account" OR $op != "logout")) {
		...
		foreach($urlins as $key => $urlin)
		{
			if ( preg_match($urlin."isU", $the_link, $matches) || preg_match($urlin."isU", rawurldecode($the_link), $matches) )
			{
				if(is_array($urlouts[$key]))
					$friendly_link = preg_replace_callback($urlin."isU", $urlouts[$key][0], $the_link);
				else
...
function parse_GT_link($REQUSERURL)
{
	global $nuke_configs, $nuke_modules_friendly_urls_cacheData;
	$parsed_vars = array();
	$unfriendly_link = '';
	
	if ($offset = strpos($REQUSERURL,'?'))
	{
		$REQUSERURL = substr($REQUSERURL,0,$offset);
	}
	else if($offset = strpos($REQUSERURL,'#'))
	{
		$REQUSERURL = substr($REQUSERURL,0,$offset);
	}
	...
	if (URL_ROOT != '/' && strrpos($REQUSERURL, URL_ROOT) !== false) $REQUSERURL=substr($REQUSERURL,strlen(URL_ROOT));

	$REQUSERURL = trim($REQUSERURL,'/');
	...
	if ($nuke_configs['gtset'] == "1")
	{
		...
		$parsed_url = (($REQUSERURL == '') || ($REQUSERURL == 'index.php')) ? array() : explode('/',html_entity_decode($REQUSERURL));

		if (file_exists(DOC_ROOT.'/'.$REQUSERURL) && ($_SERVER['SCRIPT_FILENAME'] != DOC_ROOT.$REQUSERURL) && ($REQUSERURL != '') && ($REQUSERURL != 'index.php'))
			header("location: ".LinkToGT("index.php?error=404")."");
		else
		{
			$REQUSERURL = (strrpos($REQUSERURL,".") !== false) ? $REQUSERURL:$REQUSERURL."/";
			...
			foreach( (array) $nuke_modules_friendly_urls_cacheData[1] as $key => $rewrite)
			{
				...
				{
					if ( preg_match("#^$match#is", $REQUSERURL, $matches) || preg_match("#^$match#isU", rawurldecode($REQUSERURL), $matches) )
					{
						$matched_rule = $match;
						$matched_query = $query;
						
						if(is_array($query))
							$unfriendly_link = preg_replace_callback("#^$match#isU", $query[0], $REQUSERURL);
						else
							$unfriendly_link = preg_replace("#^$match#isU", $query, $REQUSERURL);
			...
			$REQUSERURL = preg_replace("|comment-page-([0-9]{1,}+)\/|isU","", $REQUSERURL);
		}
		return array($parsed_vars, $REQUSERURL, $matched_rule, $unfriendly_link, $matched_query);
	}
	else
	{
		if(isset($_GET) && is_array($_GET))
		{
			...
			{
				$par = ($i == 0) ? "?":"&";
				$REQUSERURL .= $par."$key=$val";
				$i++;
			}
		}
		return array($_GET, $REQUSERURL, '', $REQUSERURL, '');
	}
}
...
{
	global $db, $nuke_configs, $userinfo, $visitor_ip;
	$PnValidator = new GUMP();
	$content = '';
	if($submit)
	{
		...
		elseif($mode == 'report')
		{
			...
			else
			{
				...
				$PnValidator->filter_rules(array(
					...
					'post_link'		=> 'trim|magic_quotes',
					'message'		=> 'addslashes|magic_quotes',
	...
	else
	{
		...
		else
		{
			...
			$security_code_input = makepass("_DRIEND_REPORT_FORM", $sec_code_options);
			$content .="<script src=\"".$nuke_configs['nukecdnurl']."includes/Ajax/jquery/jquery.validate.min.js\"></script>
			<form id=\"friend_form\">	
				<!-- Modal -->
				...
				<div class=\"modal-body\">
					...
					<div class=\"form-group\"> <!-- Name field -->
						...
						<input class=\"form-control\" id=\"friend_form_name\" name=\"friend_form_name\" type=\"text\" placeholder=\""._ENTER_YOUR_FRIENDNAME."\" />
					</div>

					<div class=\"form-group\"> <!-- Email field -->
						<label class=\"control-label\" for=\"friend_form_email\">ایمیل<span class=\"asteriskField\">*</span></label>
						<input class=\"form-control\" id=\"friend_form_email\" name=\"friend_form_email\" type=\"text\" placeholder=\""._ENTER_YOUR_FRIENDEMAIL."\" data-rule-required=\"true\" data-rule-email=\"true\" data-msg-email=\""._ENTER_YOUR_CORRECT_FRIENDEMAIL."\" />
					</div>

					<div class=\"form-group\"> <!-- Message field -->
						<label class=\"control-label \" for=\"friend_form_message\">"._MESSAGE."</label>
						<textarea class=\"form-control\" cols=\"40\" id=\"friend_form_message\" name=\"friend_form_message\" rows=\"3\" placeholder=\""._ENTER_MESSAGE."\"></textarea>
					</div>";
					}
					elseif($mode == 'report')
					{
					...
					<div class=\"form-group\"> <!-- Subject field -->
						...
						<input class=\"form-control\" id=\"report_form_subject\" name=\"report_form_subject\" type=\"text\" placeholder=\""._ENTER_BRIEF_TITLE."\" data-rule-required=\"true\" data-msg-required=\""._ENTER_TITLE_PLEASE."\" />
					</div>

					<div class=\"form-group\"> <!-- Message field -->
						<textarea class=\"form-control\" cols=\"40\" id=\"report_form_message\" name=\"report_form_message\" rows=\"3\" placeholder=\""._ENTER_CLEARLY_REPORTTEXT."\" data-rule-required=\"true\" data-msg-required=\""._ENTER_REPORT_TEXT_PLEASE."\"></textarea>
					</div>";
					}
					$content .="<div class=\"form-group\"> <!-- Secure Code field -->
						<label class=\"control-label \" for=\"security_code_DRIEND_REPORT_FORM\">"._SECCODE."</label>
					...
					$( \"#friend_form\" ).validate( {
						submitHandler: function () {
							$.post(\"".LinkToGT("ajax.php")."\",
							{
								...
								report_friend_form_security_code: $('#security_code_DRIEND_REPORT_FORM').val(),			
								report_friend_form_security_code_id: '_DRIEND_REPORT_FORM',			
							},
						...
						errorLabelContainer: $(\"div.errorPlacement\"),
...
function update_points($id,$uid=false)
{
    ...
    if (is_user())
	{
		...
		{
			$col_result = $db->query("SHOW COLUMNS FROM ".$users_system->users_table." LIKE 'user_points'");
			if(intval($col_result->count()) > 0)
			{
				if(isset($uid) && $uid > 0)
				{
						$result = $db->table($users_system->users_table)
							->where($users_system->user_fields['user_id'], $uid)
							->first([$users_system->user_fields['user_points']]);
				}
				elseif($username != '')
				{
						$result = $db->table($users_system->users_table)
				...
				$result[$users_system->user_fields['user_points']] = $result[$users_system->user_fields['user_points']]+$points;
				$result->save();
			}
		}
    }
...
function last_user()
{
    ...
    $result = $db->query("SELECT ".$users_system->user_fields['username']." FROM ".$users_system->users_table." WHERE ".$users_system->user_fields['user_active']." = 1 ".(($users_system->user_fields['common_where'] != '') ? " AND ".$users_system->user_fields['common_where']:"")." ORDER BY ".$users_system->user_fields['user_id']." DESC LIMIT 0,1");
	$row = $result->last();
	$lastuser = $row[$users_system->user_fields['username']];
...
function text_rel2abs_call($matches)
{
	...
	if(!preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $matches[3]))
		$matches[3] = $nuke_configs['nukeurl'].$matches[3];
...
function get_dir_list($path, $options='folders', $sort=false)
{
	$path = trim($path, "/");
	$handle=opendir($path);
	$dir_list = array();

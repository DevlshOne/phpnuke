...
include(INCLUDE_PATH."/utf_tools.php");
...
{
	if(version_compare($nuke_configs['Version_Num'], "8.4.0", ">="))
	{
	...
	{
		$db->sql_query("UPDATE ".CONFIG_TABLE." SET nukeurl='".$Req_URL."' LIMIT 1");
...
include(INCLUDE_PATH."/class.ping-optimizer.php");
...
if ((isset($lang)) AND (!stristr($lang,".")))
{
	global $currentpage;
...
if(file_exists("themes/".$nuke_configs['ThemeSel']."/theme_setup.php") && !defined("IN_INSTALL"))
	include_once("themes/".$nuke_configs['ThemeSel']."/theme_setup.php");
	
if(!defined('ADMIN_FILE') && file_exists("themes/".$nuke_configs['ThemeSel']."/theme.php") && !defined("IN_INSTALL"))
  include_once("themes/".$nuke_configs['ThemeSel']."/theme.php");
...
if(isset($_REQUEST['captcha']) && isset($_REQUEST['id']) && $_REQUEST['id'] != '')
{
	...
	if(isset($_REQUEST['id']) && filter($_REQUEST['id'], "nohtml") != '')
	{
		$requestId = adv_filter($_REQUEST['id'], array('sanitize_string'),array('alpha_dash','required'));
	...
	$language = (isset($_REQUEST['language']) && in_array($_REQUEST['language'], $language_list)) ? filter($_REQUEST['language'], "nohtml"):'farsi';
	$height = (isset($_REQUEST['height']) && intval($_REQUEST['height']) != 0) ? intval($_REQUEST['height']):50;

...
if (check_admin_permission($module_name, false, true))
{
	...
	function surveys($status='', $search_query = '', $order_by = '', $sort='DESC')
	{
		...
		if($status !== '')
		{
			...
			if($status == 0)
				$publish_now = "<a href=\"".$admin_file.".php?op=surveys_admin&mode=publish_now&pollID={POLLID}\" title=\""._PUBLISH."\" class=\"table-icon icon-5 info-tooltip\"></a>";
		}
				...
				foreach($result as $row)
				{
					...
					$contents .= "<tr>
						...
						<td align=\"center\">
							...
							<a href=\"".$admin_file.".php?op=surveys_admin&mode=delete&pollID=$pollID\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('"._DELETE_THIS_SURE."');\"></a>
							".str_replace("{POLLID}",$pollID, $publish_now)."";
							if($this_status == 2)
								$contents .= "<a href=\"".$admin_file.".php?op=surveys_admin&mode=publish_now&pollID=$pollID\" title=\""._IMMEDIATE_PUBLISH."\" class=\"table-icon icon-5 info-tooltip\"></a>";
	...
	function surveys_admin($pollID = 0, $mode = "new", $submit, $surveys_fields=array())
	{
		...
		if(isset($submit) && isset($surveys_fields) && !empty($surveys_fields))
		{
			...
			$PnValidator->filter_rules(array(
				...
				'description'		=> 'addslashes|magic_quotes',
		...
		<form action=\"".$admin_file.".php\" method=\"post\" id=\"survey_form\">
			<table width=\"100%\" class=\"id-form product-table no-border\">
				<tr>
					...
					<td><input type=\"text\" size=\"40\" name=\"surveys_fields[pollTitle]\" id=\"title_field\" value=\"".$surveys_fields['pollTitle']."\" class=\"inp-form\" minlength=\"3\" required data-msg-required=\""._TITLE_IS_REQUIRED."\" /></td>
				</tr>
				<tr>
					<th>"._PERMALINK."</th>
					<td>

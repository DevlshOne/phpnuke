			...
			if(!empty($result))
			{
				foreach($result as $row)
				{
					...
					$contents .= "<tr>
						...
						<td align=\"center\">
							<a href=\"".$admin_file.".php?op=feedbacks&mode=delete&fids=$fid\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('"._DELETE_THIS_SURE."');\"></a>
		...
		<script>
			...
			$(\".show_results\").click(function(e)
			{
				...
				$.ajax({
					type : 'post',
					url : '".$admin_file.".php',
					data : {'op' : 'reply_feedback_pm', 'fid' : fid},
	...
	function reply_feedback_pm($fid, $submit = '', $feedback_reply_message = '', $inline=0)
	{
		...
		if(isset($submit) && $submit != "" && isset($feedback_reply_message) && $feedback_reply_message != '')
		{
			...
			if($update_result)
			{
				...
				$subject = $nuke_configs['sitename']." - "._FEEDBACK.": ".$row['subject']."\n";
				...
				$msg .= "\n\r\n\r\n\r"._REPLY.": $feedback_reply_message\n\n";
				if(phpnuke_mail($to,$subject,$msg,$nuke_configs['adminmail'],$nuke_configs['sitename']))
				{
					...
					<td>
						...
						if(isset($feedback_configs['depts']) && !empty($feedback_configs['depts']))
						{
							foreach($feedback_configs['depts'] as $x2 => $depts_data)
							{
								...
								$contents .= "
								<div style=\"margin-bottom:3px;\">
									<input placeholder=\""._NAME_FAMILY."\" type=\"text\" class=\"inp-form\" value=\"$option_name\" name=\"config_fields[feedbacks][depts][$x2][name]\" />&nbsp;<input placeholder=\""._RESPONSIBILITY."\" type=\"text\" class=\"inp-form\" value=\"$option_responsibility\" name=\"config_fields[feedbacks][depts][$x2][responsibility]\" />&nbsp;&nbsp;<input placeholder=\""._EMAIL."\" type=\"text\" class=\"inp-form\" value=\"$option_email\" name=\"config_fields[feedbacks][depts][$x2][email]\" />&nbsp; &nbsp; <a href=\"#\" class=\"remove_field\">"._REMOVE."</a>
								</div>";
							}
						}
						$contents .= "

...
function check_admin_permission($filename, $only_god=false, $modules=false)
{
	...
	if ($modules)
	{
		...
		$articles_admins = ($nuke_modules_cacheData_by_title[$filename]['admins'] != '') ? explode(",", $nuke_modules_cacheData_by_title[$filename]['admins']):array();
		if(in_array($aid, $articles_admins))
			$has_permission = true;
	}
...
function adminmenu($url, $title, $image)
{
	...
	if (file_exists("themes/".$nuke_configs['ThemeSel']."/images/admin/$image"))
		$image = "themes/".$nuke_configs['ThemeSel']."/images/admin/$image";
	else
		$image = "images/admin/$image";
	
	if ($nuke_configs['admingraphic'] == 1)
		$content = "<div class = 'axe'  style = 'background-image:url($image)'></div><span>$title</span>";
	else
...
function GraphicAdmin()
{
			...
			<script type=\"text/javascript\">
				$(document).ready(function(){ 
					$.ajax({  
						type: \"POST\",  
						url: \"".$admin_file.".php\",  
						data: { op: \"get_latest_vershion_from_nuke\", mode : \"adminmain\"},  
	...
	if(sizeof($main_menus) > 1 || !empty($module_menus))
	{
		...
		$admin_menus['logout'] = adminmenu("".$admin_file.".php?op=logout", ""._ADMIN_LOGOUT."", "logout.png");	
...
function adminMain()
{
	global $nuke_configs, $admin, $aid, $db, $admin_file, $sort, $nuke_authors_cacheData, $nuke_modules_cacheData, $users_system, $pn_dbname;
	...
	{
		$result1 = $db->query("SELECT ".$users_system->user_fields['user_regdate']." FROM ".$users_system->users_table." WHERE ".$users_system->user_fields['user_regdate']." >= '$last_month' AND ".$users_system->user_fields['user_regdate']." <= '$now_time' AND ".$users_system->user_fields['common_where']."");
					...
					if($db->count() > 0)
					{
						foreach($result as $row)
						{
							...
							$contents .= "<tr style=\"border-bottom:1px solid #eee;\">
								...
								<td align=\"center\">
									<a href=\"".$admin_file.".php?op=comments_delete&cid=$cid\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('"._SURETODELCOMMENTS."');\"></a>
									<a href=\"".$admin_file.".php?op=comments_status&cid=$cid&new_status=1\" title=\""._APPROVE."\" class=\"table-icon icon-5 info-tooltip\"></a>
					...
					if($db->count() > 0)
					{
						foreach($result as $row)
						{
							...
							$contents .= "<tr style=\"border-bottom:1px solid #eee;\">
								...
								<td align=\"center\">
									<a href=\"".$admin_file.".php?op=feedbacks&mode=delete&fids=$fid\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('"._FEEDBACK_DELETE_CONFIRM."');\"></a>
		...
		<div class=\"admin_main_box fr\" style=\"width:47%;\">
			...
			<div class=\"admin_main_box_content\">
				<table width=\"100%\" class=\"product-table min_table no-border\">
					...
					if($users_table_exists)
					{
						$result = $db->query("SELECT ".$users_system->user_fields['user_id'].", ".$users_system->user_fields['username'].", ".$users_system->user_fields['user_regdate']." FROM ".$users_system->users_table."".(($users_system->user_fields['common_where'] != '') ? " WHERE ".$users_system->user_fields['common_where']."":"")." ORDER BY ".$users_system->user_fields['user_id']." DESC LIMIT 0,5");
						$rows = $result->results();

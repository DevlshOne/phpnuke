...
if (check_admin_permission($filename))
{
	...
	function reports($module_name='', $post_id=0, $post_title='', $mode = '', $rids, $search_query = '', $order_by = '', $sort='DESC')
	{
		...
		$contents .= "<div align=\"center\" style=\"margin:20px 0;\">"._VIEW_REPORTS_IN_MODULE." <select onchange=\"top.location.href=this.options[this.selectedIndex].value\" class=\"styledselect-select\"><option value=\"".$admin_file.".php?op=reports\">همه</options>\n".implode("\n", $all_modules_reports_link)."</select></div>";
		...
		$result = $db->query("
			SELECT r.*, 
			u.".$users_system->user_fields['username'].",

				...
				foreach($result as $row)
				{
					...
					$contents .= "<tr>
						...
						<td align=\"center\">
							...
							<a href=\"".$admin_file.".php?op=nav_menus_admin&mode=delete&nav_id=$nav_id\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('"._NAV_DELETE_SURE."');\"></a>
						</td>
						</tr>";
				}
			}
		...
		if($mode == "add_nav_menu" && !empty($nav_menu_fields))
		{
			...
			elseif($nav_menu_fields['type'] == 'categories')
			{
				...
				if(!empty($nav_categories))
				{
					...
					{
						$insert_values[] = array($new_increment_nid, $nav_id, $cat_text, $nav_menu_fields['type'], $nav_menu_fields['module'], $cat_id);
						$new_increment_nid++;
					}
				}
				...
				if($insert_values != '')
					$insert = $db->table(NAV_MENUS_DATA_TABLE)
									->multiinsert(["nid", "nav_id", "title", "type", "module", "part_id"],$insert_values);
				else
					$insert = false;
			}

...
if (check_admin_permission($filename))
{
	...
	function ip_ban_page($id=0,$ipsearch=array(), $ipsearch2, $ipaddress='')
	{
		...
		<input type='hidden' name='op' value='addnewip'><input type='hidden' name='edited_id' value='$id'></form></div>
			...
			foreach($nuke_mtsn_ipban_cacheData as $id => $nuke_banned_ip_info){
				...
				$contents .= "<tr>
					...
					<td align='center'>
						...
						<a href=\"".$admin_file.".php?op=deleteip&id=$id\" class=\"table-icon icon-5 info-tooltip\" title=\""._UNBAN."\" onclick=\"return confirm('"._UNBAN_CONFIRM."');\"></a>

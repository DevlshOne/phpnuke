...
if (check_admin_permission($filename))
{
	...
	function comments($status=1, $reported=0, $module_name='', $post_id=0, $post_title='')
	{
		...
		$contents .= "
			...
			$link_to = "".$admin_file.".php?op=comments".(($status != 0) ? "&status=$status":"").(($reported != 0) ? "&reported=$reported":"").(($module_name != '') ? "&module_name=$module_name":"").(($post_id != 0) ? "&post_id=$post_id&post_title=$post_title":"");
			...
			if($db->count() > 0)
			{
				foreach($result as $row)
				{
					...
					$username_link = ($username == $ip) ? $ip_info:LinkToGT("index.php?modname=User_Account&username=$username");
					...
					$contents .= "<tr>
						...
						<td align=\"center\">
							...
							<a href=\"".$admin_file.".php?op=comments_delete&cid=$cid&status=$status\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\"></a>
							<a href=\"".$admin_file.".php?op=comments_status&cid=$cid&new_status=$new_status\" title=\"$new_status_desc\" class=\"table-icon icon-$new_status_icon info-tooltip\"></a>
		...
		<input type=\"hidden\" name=\"op\" value=\"comments_delete\">
		<div class=\"text-center\"><input type=\"submit\" value='"._DELETE_SELECTED."' onclick=\"return confirm('"._DELETE_THIS_SURE."');\"></div><br /><br />
	...
	function comments_edit($cid, $update='', $comment_fields='', $show_header=0)
	{
		...
		$content="
			...
			</table><input type=\"hidden\" name=\"op\" value=\"comments_edit\"><input type=\"hidden\" name=\"cid\" value=\"$cid\"></form>";
			if($show_header == 1)
			{
				$contents = '';
				include("header.php");
	...
	function comments_delete($cid = 0, $cids = array(), $comment_username='')
	{
	...
	function comments_status($cid, $new_status, $comment_username='')
	{
		global $db, $admin_file, $nuke_configs;
		...
		if($cid != 0)
		{
			$where = "(cid = '$cid' OR pid = '$cid')";
		}
				
		$result = ($where != '') ? $db->query("UPDATE ".COMMENTS_TABLE." SET status = '$new_status' WHERE ".$where.""):"";
		phpnuke_db_error();
		header("location: ".$admin_file.".php?op=comments");
		die();
	}
	...
	{
		global $db, $admin_file, $nuke_configs, $nuke_authors_cacheData, $userinfo, $aid, $visitor_ip;
		$cid = intval($cid);
		
		$row = $db->table(COMMENTS_TABLE)
					->where("cid", $cid)
					->select(["comment", "module", "post_id", "post_title"])
					->first();
		...
		if(isset($reply_save) && $reply_save != "" && isset($reply) && $reply != '')
		{
			...
			$commenter_url		= isset($userinfo['user_website']) ? $userinfo['user_email']:$nuke_authors_cacheData[$aid]['url'];
	...
	switch ($op) {
		...
		break;
		case "comments_edit":
			comments_edit($cid, $update, $comment_fields, $show_header);
		break;
		case "comments_delete":
			comments_delete($cid, $cids, $comment_username);
		break;
		case "comments_status":
			comments_status($cid, $new_status);
		break;
		case "comments_reply":

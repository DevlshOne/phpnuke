...
if (check_admin_permission($filename, true))
{
	...
	function displayadmins($user_id=0)
	{
		...
		if (is_God())
		{
			if($user_id != 0)
			{
				$result = $db->query("SELECT ".$users_system->user_fields['username'].", ".$users_system->user_fields['name'].", ".$users_system->user_fields['user_email'].", ".$users_system->user_fields['user_password']." FROM ".$users_system->users_table." WHERE ".$users_system->user_fields['user_id']."= ?", [$user_id]);
				
				$row = (isset($result->results()[0])) ? $result->results()[0]:array('','','','','');
				list($uname, $rname, $remail, $rsite, $rupass) = $row;
			}
			else
			...
			if($db->count() > 0)
			{
				foreach ($result as $row)
				{
					...
					{
						$contents .= "<a class=\"table-icon icon-2 info-tooltip\" href=\"".$admin_file.".php?op=deladmin&amp;del_aid=$a_aid\" title=\""._DELETE_ADMIN."\"></a></td></tr>";
					}
				}
			}
			$contents .= "</table>
			...
			<script>
				$(document).ready(function(){
					$(\".tagit-close\").on('click', function(){
						...
						$.post(
						'".$admin_file.".php',
						{'op' : 'remove_permission', 'admin_id' : admin_id, 'amid' : amid, 'mid' : mid , 'type' : type },
						function(data)
						{
							if(data == 'true')
							{
	...
	function deladmin($del_aid)
	{
		...
		$contents .= "[ <a href=\"".$admin_file.".php?op=deladmin2&amp;del_aid=$del_aid\">" . _YES . "</a> | <a href=\"".$admin_file.".php?op=mod_authors\">" . _NO . "</a> ]</div>";
	...
	function deladmin2($del_aid)
	{
		...
		if (is_God())
		{
			...
			if ($radminarticle == 1)
			{
				//$db->sql_query("SELECT sid from ".ARTICLES_TABLE." where aid='$del_aid'");
				$aid_articles = $db->table(ARTICLES_TABLE)
								->where("aid", $del_aid)
								->select(['sid'])
								->count();
				...
				if ($aid_articles > 0)
				{
					...
					."<input type=\"hidden\" name=\"op\" value=\"assignarticles\">"
	...
	function assignarticles($del_aid, $newaid)
	{
		...
		$numrows = $db->table(ARTICLES_TABLE)
						->where("aid", $del_aid)
						->select(['sid'])
						->count();

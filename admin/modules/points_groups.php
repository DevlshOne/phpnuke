			...
			if($db->count() > 0)
			{
				foreach($result as $row){
					...
					<tr>
						...
						".(($row['type'] == 0) ? "<a href=\"".$admin_file.".php?op=group_edit&id=$id\" class=\"table-icon icon-1 info-tooltip\" title=\""._EDIT."\"></a><a href=\"".$admin_file.".php?op=group_del&id=$id\" class=\"table-icon icon-2 info-tooltip\" title=\""._DELETE."\" onclick=\"return confirm('"._POINT_GROUP_DELETE_SURE."');\"></a>":"")."
						</td>
					</tr>
					</form>";
				}

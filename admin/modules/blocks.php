...
if (check_admin_permission($filename)) {
	...
	function add_remove_blocks()
	{
		...
		$exept_files = array(".", "..", ".htaccess","index.hml");
		$handle=opendir('blocks');
		while ($file = readdir($handle))
		{
			$file_extention = explode(".", $file);
			if(!in_array($file, $exept_files) && end($file_extention) == 'php' && preg_match("#block-#", $file) && !is_dir("blocks/$file"))
			{
				$blockslist[] = $file;
			}
		}
			...
			$(document).ready(function(){ 
				$(function() {
					$(\"#contentLeft table tbody\").sortable({ opacity: 0.6, cursor: 'move', update: function() {
						var order = 'action=updateRecordsListings&' + $(this).sortable(\"serialize\");
	...
	function BlocksEdit($box_id, $bid, $block_type, $all_box_ids)
	{
		...
		$(\"#edit-form-submit\").click(function(){
			...
			$.ajax({
				type : 'post',
				url : '".$admin_file.".php',
				data : {'op' : 'BlocksSave', 'block_fields' : block_fields, box_id : box_id, bid : '$bid', new_block : new_block},
	...
	function HeadlinesAdmin()
	{
		...
		foreach ($nuke_headlines_cacheData as $hid => $nuke_headlines_info) {
			...
			."<td align=\"center\"><a href=\"".$admin_file.".php?op=HeadlinesEdit&amp;hid=$hid\" class=\"table-icon icon-1 info-tooltip\" title=\""._EDIT."\"></a> <a class=\"table-icon icon-2 info-tooltip\" href=\"".$admin_file.".php?op=HeadlinesDel&amp;hid=$hid&ok=1\" onclick=\"return confirm('"._CONFIRM_HEADLINE_DELETE."');\" title=\""._DELETE."\"></a></td><tr>";
		}
		$contents .= "</form></td></tr></table>";
		$contents .= CloseAdminTable();
		$contents .= "<br />";
		$contents .= OpenAdminTable();

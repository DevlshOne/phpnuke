...
if (check_admin_permission($filename))
{
	function upgrade($start_upgrade)
	{
					...
					if(!$version_check)
					{
						...
						$contents .= "<tr>
							...
							<form action=\"http://www.phpnuke.ir/upgrader.php\" method=\"post\" id=\"infoForm\">
								...
								<input type=\"hidden\" name=\"redirect_url\" value=\"".$nuke_configs['nukeurl']."".$admin_file.".php?op=upgrade&start_upgrade=true\" />
							</form>
							</td>
						</tr>";
					}
		...
		else
		{
			...
			$zip_dir = @basename("PHPNukeMTEdition-".$latest_version.".zip");

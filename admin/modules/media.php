...
if (check_admin_permission($filename))
{
	...
	function common()
	{
		...
		[ <a href=\"".$admin_file.".php?op=media_browser\">"._FILES_MANAGER."</a> | <a href=\"".$admin_file.".php?op=media_upload&upload_dir=files/uploads\">"._UPLOAD_FILE."</a> ]
		</p>
		";
	...
	function media_browser($ckeditor='')
	{
		...
		<table width=\"100%\" class=\"product-table no-hover\">
			...
			<tr>
				...
				<td style=\"vertical-align:top;\"><a href=\"".$admin_file.".php?op=media_upload&upload_dir=$default_folder&ckeditor=$ckeditor\" id=\"uploadto\">"._UPLOAD_IN."<br /><span id=\"upload_path\" dir=ltr>$default_folder</span></a><div id=\"media_menu\"></div></td>
			</tr>
		</table>
		<script>
	...
	function media_upload($upload_dir, $ckeditor)
	{
		...
		if(isset($_FILES['file_upload']))
		{
			...
			if($_FILES['file_upload']['error'] == 0)
			{
				...
				$filename = substr($_FILES['file_upload']['name'], 0, strrpos($_FILES['file_upload']['name'], "."));
				...
				if(move_uploaded_file($_FILES['file_upload']['tmp_name'], $upload_dir.$file_name))
				{
					...
					die('{"status":"success","message":"'._UPLOAD_SUCCESS.'","file_name":"'.$file_name.'"}');
					exit;
				}
				else
				{
	...
	function get_media_metadata($file_name)
	{
		...
		{
			$contents .= "<img src=\"index.php?timthumb=true&src=$file_name&w=390\" width=\"390\" style=\"border:1px solid #ccc;padding:2px;\" />";
		}
		...
		{
			$contents .= "<video width=\"390\" poster=\"index.php?timthumb=true&data_image=true&src=$file_name\" height=\"250\" controls=\"controls\" preload=\"none\">
	...
	function media_get_files($directory, $page, $ckeditor)
	{
		...
		{
			$filename = basename($maindir."/".$filename);
			
			if(mb_detect_encoding($maindir."/".$filename) == "UTF-8")
			{
				//$filename = iconv("windows-1256", "utf-8//TRANSLIT//IGNORE", basename($maindir."/".$filename));
			}
		...
		for($i=$offset;$i<$newICounter;$i++)
		{
			...
			$files_info[] = array(
				'name' => $all_files[$i],
				'urlname' => ((mb_detect_encoding($all_files[$i]) == "UTF-8") ? iconv("windows-1256", "utf-8//TRANSLIT//IGNORE", $all_files[$i]):$all_files[$i]),

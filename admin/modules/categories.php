...
if (check_admin_permission($filename))
{
	...
	function categories($module_name='Articles', $parent_id=0)
	{
		...
		if($parent_id != 0)
		{
			$cat_title = sanitize(filter(implode("/", array_reverse(get_parent_names($parent_id, $nuke_categories_cacheData[$module_name], "parent_id", "catname_url"))), "nohtml"), array("/"));
			...
			$cats_link_deep = preg_replace('#<a(.*)href="(.*)"(.*)id="(.*)">(.*)</a>#isU', '<a$1href="'.$admin_file.'.php?op=categories&module='.$module_name.'&parent_id=$4"$3>$5</a>', $cats_link_deep);
			
			$cats_link_deep = " - <a href=\"".$admin_file.".php?op=categories&module_name=$module_name\">"._MAIN_CATS."</a>/$cats_link_deep";
		}
		$pagetitle = sprintf(_CATS_ADMIN_PART, $module_name).$cats_link_deep;
		...
		{
			$sel = ($module_name == $modules_category) ? "selected":"";
			$all_modules_categories_link[] = "<option value=\"".$admin_file.".php?op=categories&module_name=$modules_category\" $sel>$modules_category</option>";
		}
		
		$contents .= "<div align=\"center\" style=\"margin:20px 0;\">[ <a href=\"#\" title=\""._ADD_CAT."\" class=\"editindialog\" data-op=\"categories_admin\" data-catid=\"0\" data-module_name=\"$module_name\">"._ADD_CAT."</a> ]<br /><br />"._SHOW_MODULES_CAT." <select onchange=\"top.location.href=this.options[this.selectedIndex].value\" class=\"styledselect-select\">".implode("\n", $all_modules_categories_link)."</select></div>";
		...
		$contents .= "
			...
			$link_to = "".$admin_file.".php?op=categories&module_name=$module_name";
			...
			if($db->count() > 0)
			{
				foreach($result as $row)
				{
					...
					$module_link_to = $admin_file.".php?op=categories&module_name=$module&parent_id=$catid";
					...
					$contents .= "<tr>
						...
						<td align=\"center\">
							<a href=\"#\" title=\""._EDIT."\" class=\"table-icon icon-1 info-tooltip editindialog\" data-op=\"categories_admin\" data-catid=\"$catid\"></a>
							".(($type != 1) ? "<a href=\"".$admin_file.".php?op=categories_delete&catid=$catid&module_name=$module\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\"></a>":"")."
		...
		<script>
			$(\".editindialog\").click(function(e)
			{
				...
				$.ajax({
					type : 'post',
					url : '".$admin_file.".php',
					data : {'op' : op, 'catid' : catid, 'module_name' : module_name},
	...
	function categories_admin($catid=0, $update='', $cat_fields='', $module_name='Articles')
	{
		...
		$content="
			...
			</table><input type=\"hidden\" name=\"op\" value=\"categories_admin\"><input type=\"hidden\" name=\"catid\" value=\"$catid\"><input type=\"hidden\" name=\"module_name\" value=\"$module_name\"></form>";
			$content .=  jquery_codes_load('',true);
			die($content);
	}

	function categories_delete($catid, $module_name='Articles')
	{
		global $db, $admin_file, $nuke_configs;
		...
		if($nuke_categories_cacheData[$module_name][$catid]['type'] == 1)
		{
			header("location: ".$admin_file.".php?op=categories");
			die();
		}
		
		$module = $nuke_categories_cacheData[$module_name][$catid]['module'];
		...
		$parent_id = $nuke_categories_cacheData[$module_name][$catid]['parent_id'];
	...
	$module_name = (isset($module_name)) ? filter($module_name, "nohtml"):'Articles';
	...
	switch ($op) {
		case "categories":
			categories($module_name, $parent_id);
		break;
		case "categories_admin":
			categories_admin($catid, $update, $cat_fields, $module_name);
		break;

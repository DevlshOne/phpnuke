...
if (check_admin_permission($filename))
{
	function modules()
	{
		...
		foreach($nuke_modules_cacheData as $mid => $module_info)
		{
			...
			if ($module_info['main_module'] == 1) {
				...
				$puthome = "<a href='".$admin_file.".php?op=home_module&mid=$mid' onclick=\"retirn confirm('"._SURETOCHANGEMOD."');\">"._PUTINHOME."</a>";
	...
	function module_edit($mid, $submit='', $module_data=array())
	{
		...
		if(isset($submit) && $submit != '' && is_array($module_data) && !empty($module_data))
		{
			...
			if($nuke_configs['multilingual'] == 1){
				$lang_titles = (!empty($module_data['lang_titles'])) ? addslashes(phpnuke_serialize($module_data['lang_titles'])):"";
			}else{
		...
		$module_files_list = get_dir_list('modules/'.$modules_row['title'], 'files');

		if(is_array($module_files_list) && !empty($module_files_list))
		{
			foreach($module_files_list as $key => $val)
			{
				if($val == "copyright.php")
					unset($module_files_list[$key]);
				$val2 = explode(".", $val);
				if(end($val2) != 'php')
					unset($module_files_list[$key]);
				if(substr($val, 0, 3) == "GT-")
					unset($module_files_list[$key]);
				if($val == "config_function.php")
					unset($module_files_list[$key]);
			}
			
			$module_files_list = array_filter($module_files_list);
		}
		$module_files_list2 = array();
		
		if(is_array($module_files_list) && !empty($module_files_list))
		{
			foreach($module_files_list as $module_file)
			{
				$module_file = str_replace(".php", "", $module_file);
				$module_files_list2[] = "<a href=\"".$admin_file.".php?op=module_edit_boxess&mid=$mid&module_file=$module_file\">$module_file</a>";
			}
		}
		$module_files_list2 = "[ ".implode(" | ", $module_files_list2)." ]";
	...
	function module_edit_boxess($mid, $module_file='', $submit='', $module_boxes='')
	{
		global  $pagetitle, $admin_file, $db, $nuke_configs;
		$mid = intval($mid);	
		...
		{
			$module_file = str_replace(".php","", $module_file);
			$all_module_boxes[$module_file] = $module_boxes;
			$all_module_boxes = addslashes(phpnuke_serialize($all_module_boxes));
		...
		$module_boxes_def = (isset($all_module_boxes[$module_file])) ? $all_module_boxes[$module_file]:'';
		
		$all_module_boxes = (isset($all_module_boxes[$module_file])) ? explode("|", $all_module_boxes[$module_file]):array();
		$all_boxes = array();
		...
		$contents .= "<div align=\"center\"><form action='".$admin_file.".php' method='post'>
		<table align='center' border='0' cellpadding='2' width=\"100%\" cellspacing='2' class=\"id-form product-table no-border\">
		<tr><th align='center' class='option' style=\"text-align:center\"><b>"._CHANGEMODEDIT."</b><br>(".$modules_row['title']." - "._FILE." $module_file)<br /><br /><a href=\"".$admin_file.".php?op=module_edit&mid=$mid\">"._BACKTOMODULEEDITPAGE."</a></th></tr>";
		//////////////////////
		...
		<input type='hidden' name='mid' value='$mid'>
		<input type='hidden' name='module_file' value='$module_file'>
	...
	$module_boxes = (isset($module_boxes)) ? addslashes($module_boxes):'';
	$module_file = (isset($module_file)) ? filter($module_file, "nohtml"):'';
	...
	switch ($op)
	{
		...
		case "module_edit_boxess":
			module_edit_boxess($mid, $module_file, $submit, $module_boxes);
		break;
	}
}
else

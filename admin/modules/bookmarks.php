...
if (check_admin_permission($filename)) {
	...
	function bookmarks() {
		...
		<div align=\"center\"><br /><br /><br /><br />
			<form action=\"".$admin_file.".php\" method=\"post\">
				...
				<input type=\"hidden\" name=\"op\" value=\"save_bookmarks\"></form>
			</div>
			...
			foreach($nuke_bookmarksite_cacheData_by_weight as $weight => $nuke_bookmarksite_info){
				...
				$contents .= "<tr id=\"bookmark_$bid\">
					...
					<td>$title</td>
					<td><a href=\"".$admin_file.".php?op=editbookmark&bid=$bid\" title=\""._EDIT."\" class=\"table-icon icon-1 info-tooltip\"></a> <a href=\"".$admin_file.".php?op=save_bookmarks&delete=1&bid=$bid\" title=\""._DELETE."\" class=\"table-icon icon-2 info-tooltip\"></a></td>
					</tr>";
			}
			$contents .= "
			</tbody>
				...
				$('#bookmarks-table tbody').sortable({
					...
					update: function(){
						...
						$.ajax({
							type : 'post',
							url : '".$admin_file.".php',
							data : {'op' : 'updatebookmarksweight', 'bookmarks_ids': arr},
	...
	function editbookmark($bid) {
		...
		$contents .= "
			...
			</table><input type=\"hidden\" name=\"op\" value=\"save_bookmarks\"><input type=\"hidden\" name=\"bid\" value=\"$bid\"></form></div>
		</table>";

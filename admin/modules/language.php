...
if (check_admin_permission($filename))
{
	...
	function language($new_word="", $new_word_equals="", $word_search="")
	{
		...
		for($i=$start_at; $i < $newICounter; $i++)
		{
			...
			$equal_in_current_lang = $nuke_languages[$nuke_configs['currentlang']][$word];
			...
			$contents .= "<tr>
				...
				<td align=\"center\">";
					if(isset($nuke_languages_cacheData[$word]) && !empty($nuke_languages_cacheData[$word]))
						$contents .= " <a class=\"table-icon icon-2 info-tooltip\" href=\"".$admin_file.".php?op=delete_language_word&word=$word\" title=\""._DELETE."\" onclick=\"return confirm('"._DELETE_PHRASE_CONFIRM."');\"></a>";

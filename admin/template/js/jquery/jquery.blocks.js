...
$(document).ready(function(){
	...
	$("body").on('click','.blocks-tools a', function(){
		...
		if(operation == 'preview')
		{
			...
			$.ajax({
				type : 'post',
				url : ''+admin_file+'.php',
				data : {'op' : 'preview_block', 'bid' : bid, 'box_id' : box_id},
		...
		if(operation == 'infblock')
		{
			...
			$.ajax({
				type : 'post',
				url : ''+admin_file+'.php',
				data : {'op' : 'info_block', 'bid' : bid, 'box_id' : box_id},
				success : function(responseText){
					$("#blocks-dialog").html(responseText);
		...
		if(operation == 'remove')
		{
			...
			if(remove_confirm == true){
				...
				$.ajax({
					type : 'post',
					url : ''+admin_file+'.php',
					data : {'op' : 'remove_block', 'bid' : bid, 'box_id' : box_id},
					success : function(responseText){
		...
		if(operation == 'active')
		{
			...
			$.ajax({
				type : 'post',
				url : ''+admin_file+'.php',
				data : {'op' : 'change_block_status', 'bid' : bid, 'box_id' : box_id},
				success : function(responseText){
					responseText = typeof responseText === 'object' ? responseText : JSON.parse(responseText);
					if(responseText.status == 'success')
					{
		...
		if(operation == 'edit')
		{
			...
			$.ajax({
				type : 'post',
				url : ''+admin_file+'.php',
				data : {'op' : 'BlocksEdit', 'bid' : bid, 'box_id' : box_id},
				success : function(responseText){
					$("#blocks-dialog").html(responseText);
		...
		if(remove_confirm == true)
		{
			...
			if(remove_box_id != 'left' && remove_box_id != 'right' && remove_box_id != 'topcenter' && remove_box_id != 'bottomcenter')
			{
				...
				$.ajax({
					type : 'post',
					url : ''+admin_file+'.php',
					data : {'op' : 'updateweight', 'mode' : 'remove', 'box_id': remove_box_id},
	...
	$("body").on('click','#edit_box_id_submit', function(){
		...
		$.ajax({
			type : 'post',
			url : ''+admin_file+'.php',
			data : {'op' : 'updateweight', 'mode' : 'edit' , 'box_id': edit_dialog_box_id, 'new_box_title': new_box_title.replace(' ', '_'), 'new_box_status': new_box_status, 'new_box_theme_location': new_box_theme_location, 'new_box_theme_priority': new_box_theme_priority},
			success: function(theResponse){
				theResponse = typeof theResponse === 'object' ? theResponse : JSON.parse(theResponse);
				if(theResponse.status == 'success')
				{
	...
	$("#add-html-block, #add-rss-block").on('click', function(){
		...
		$.ajax({
			type : 'post',
			url : ''+admin_file+'.php',
			data : {'op' : 'BlocksEdit', block_type : block_type[1], all_box_ids : all_box_ids},
			success : function(responseText){
				$("#blocks-dialog").html(responseText);

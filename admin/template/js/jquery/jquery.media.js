    ...
    $.fn.medias = function(options)
	{
		...
		$( self ).on('click', 'li a', function()
		{
			...
			if( entry.hasClass('collapsed') )
			{
				...
				getfile( this_dir, 1 );
				$('#uploadto').attr('href', media_settings.admin_file+'.php?op=media_upload&upload_dir='+this_dir+((media_settings.ckeditor == 'true') ? '&ckeditor='+media_settings.ckeditor+'':''));
...
function getfilelist( cont, root )
{
	...
	$.post( media_settings.admin_file+'.php', { 'op' : 'media_get_menu_files', dir: root }, function( data )
	{
...
function getfile( this_dir, page)
{
	...
	$.ajax({
		...
		data: {'op': 'media_get_files' , 'directory': this_dir , 'page': page, 'ckeditor' : media_settings.ckeditor}, success: function (responseText) {
			...
			$.each(medias_data.files, function(index, value){
				...
				{
					img_src = '<img src="index.php?timthumb=true&src='+this_dir+'/'+value.urlname+'&w='+thumb_width_height+'&h='+thumb_width_height+'" width="'+thumb_width_height+'" class="'+value.mime_type+'" height="'+thumb_width_height+'" rel="'+this_dir+'/'+value.urlname+'" style="border:1px solid #ccc;padding:1px;" title="'+value.urlname+'" />';
				}
				else
				{
...
function delete_file( filename, page, is_multi)
{
	...
	if((filenames[0] != '' && is_multi == 1) || is_multi == 0)
	{
		...
		$.ajax({
			type: "POST",
			url: media_settings.admin_file+'.php',
			data: {'op': 'delete_media' , 'file_name': filenames}, success: function (responseText) {
				medias_data = typeof responseText === 'object' ? responseText : JSON.parse(responseText);
